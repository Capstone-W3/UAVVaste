...
class ConvertCOCOToYOLO:
    ...
    def __init__(self, img_folder, json_path, write_path):
    ...
    def convert(self,annotation_key='annotations',image_key='images',img_id='image_id',cat_id='category_id',bbox_id='bbox'):
        ...
        for i in range(len(data[annotation_key])):
            ...
            bbox = data[annotation_key][i][bbox_id]

            image_name = str(data[image_key][image_id]["file_name"])

            # Retrieve image.
            if self.img_folder == None:
                image_path = '%s' % image_name
            else:
                image_path = './%s/%s' % (self.img_folder, image_name)
            ...
            filename =  '%s.txt' % str(image_name)[:-4]
            content = '%s %s %s %s %s' % (str(category_id), str(yolo_bbox[0]), str(yolo_bbox[1]), str(yolo_bbox[2]), str(yolo_bbox[3]))

            # content =f"{category_id} {yolo_bbox[0]} {yolo_bbox[1]} {yolo_bbox[2]} {yolo_bbox[3]}"

            write_file = '%s/%s' % (self.write_path, filename)
...
if __name__ == "__main__":
    ConvertCOCOToYOLO(img_folder='./images',json_path='./annotations/annotations.json', write_path='./labels').convert()